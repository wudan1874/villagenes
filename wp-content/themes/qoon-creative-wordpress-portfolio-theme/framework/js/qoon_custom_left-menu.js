jQuery.noConflict()(function($){
	...
	/*FIRST TIME LOAD*/
		...
		if($('body').outerWidth()< 1919){
			if($('body').outerWidth()< 1100){
				var ws = $('.oi_header_side').outerWidth()
			}
			else{
				var ws = $('.oi_header_side').outerWidth()/20*3;
			}
		}else{
			var ws = 288;
		}
		var is = $(window).width();
		$('.oi_footer').css('max-width', ws-50)
		$('.oi_tag_line_holder').css('max-width', ws-55)
		$('.oi_header_side').css('width','100%');
		if($('body').outerWidth()< 1400){
			$('.oi_tag_line_holder').css('max-width', ws-30)
		}
		...
		$(window).load(function() {
			if(oi_theme.show_ajax ==1){
			if(check_url.if_home == 'no')
			{
				if($('body').outerWidth()< 1100){
					$('.oi_header_side').css({'margin-left': -ws});
					$('.oi_header_side.oi_full_page').css({'display': 'block'});
				}
			}
			setTimeout(function(){
			  if(!$('body').hasClass('oi_will_be_full_page')){
				if($('body').outerWidth()< 1100){
					if(check_url.if_home == 'yes')
					{
						$('.oi_header_side.oi_full_page').css({'display': 'block'});
...
/*MENU*/
	...
	$('#menu-item-2859 > a').click(function(e) {
	...
	$('#menu-item-2910 > a').click(function(e) {
		$(this).parent().find(' > ul').prepend('<li class="oi_go_back"><span><i class="fa fa-long-arrow-left"></i> '+$(this).html()+'</span></li>');
		$(this).parent().parent().find('> li > a').css('pointer-events', 'none').animate({'opacity':0}, 100);
		$(this).parent().find('> ul').css('display','block').animate({'opacity':1	,'margin-left':0}, 400);
		
		$('.oi_go_back').click(function(e) {
			$(this).parent().animate({'opacity':0}, 100).css('display','none').css('margin-left','100px');
			$(this).parent().parent().parent().find('>li > a').animate({'opacity':1}, 100).css('pointer-events', 'auto');
			$(this).remove();
		});
		e.preventDefault();
	});
...
/*************************************/	
	// $(window).load(function() {
	//     var bg = $('#oi_current_image').css('background-image').replace('url(','').replace(')','').replace(/\"/gi, "");
	// 	$('body.custom-background').css('background-image', 'url('+bg+')').css('background-color', 'rgba(0,0,0,0.5)').css('background-blend-mode', 'multiply');
	// });
/*************************************/		
/*************************************/	
/*CHANGE PAGE AJAX*/
	...
	$('.oi_creative_p_holder .oi_c_resize').click(function(e) {
	...
	$('.oi_crea_a_partner').click(function(e) {
		var first = $('.oi_creative_p_content_partner').attr('data-first');
		var last = $('.oi_creative_p_content_partner').attr('data-last');
		var tags = $('.oi_creative_p_content_partner').attr('data-tags');
		var category = $('.oi_creative_p_content_partner').attr('data-category');
		var group = $('.oi_creative_p_content_partner').attr('data-group');

		var arr_n = JSON.parse(group);
		var arr = new Array();

		for(var i=0; i<arr_n.length;i++)
		{
			arr.push(JSON.stringify(arr_n[i]));
		}
		...
		$('.oi_creative_p_content_partner').animate({'opacity': 0,'bottom':'100px'	}, 100);
		
		$.get
			  (
			  oi_theme.ajax_url,{'action': 'qoon_ajax_request_p', 'id' : id.toString()},function(result,status)
				{
					$(result.new_posts).imagesLoaded( function(){
						...
						$('.oi_creative_p_content_partner').css({'bottom':'50px'})

						$('.oi_prev_c_p_partner').attr('data-id',arr[prev]);
						$('.oi_next_c_p_partner').attr('data-id',arr[next]);
						$('.oi_c_title a').html(result.new_posts.title);
						$('.oi_c_date').html(result.new_posts.date);
						$('.oi_c_cats').html(result.new_posts.cat);
						$('.oi_c_description_content_partner').html(result.new_posts.descr);
						$('.oi_c_details').attr('href',result.new_posts.details);
						$('.oi_c_title a').attr('href',result.new_posts.details);
						$('.oi_c_title a').attr('data-id',result.new_posts.cur);
						$('.oi_c_details').attr('data-id',result.new_posts.cur);
						...
						$('.oi_prev_c_p_partner').attr('data-id',result.new_posts.prev);
						$('.oi_next_c_p_partner').attr('data-id',result.new_posts.next);
						...
						setTimeout(function(){$('.oi_creative_p_content_partner').animate({'opacity': 1}, 300)}, 360);
...
if(oi_theme.show_ajax ==1){	
	if(oi_theme.is_blog || oi_theme.is_blog_t){
		$('.oi_list_cats a, a.oi_post_image:not(.oi_must_zoom), a.blog_title_a, .oi_readmore_btn, .page-numbers').click(function(e) {
			...
			$('.oi_loader').find('.percent').html('0%');
			$('.oi_loader').find('.water').css({"transform":"translate(0,100%)"});
...
$({numValue: 1}).animate({numValue: 100}, {
  ...
  easing:'swing',
  step: function() {
    $('.percent').text(Math.ceil(this.numValue)+'%');
    $('.water').css({"transform":"translate(0"+","+ Math.ceil(100 - this.numValue) +"%)"});
  }
});
