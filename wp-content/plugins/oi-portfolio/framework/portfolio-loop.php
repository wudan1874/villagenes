...
$p_ptag = "";
if  ($e_ptag !='All'){
$p_ptag = get_term_by('name', $e_ptag, 'portfolio-product-tags');
};
			...
			if($p_category !=''){
				$args = array(
					...
					'tax_query' => array(
						...
						array(
							'taxonomy' => 'portfolio-product-tags',
							'terms'    => $p_ptag->term_id,
						),
						array(
...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
    ...
    <?php $thumb_image_url = wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), '') ?>
        ...
        <?php 
        if($post->post_name == 'holding_block_wide' || $post->post_name == 'holding_block_square' || stristr($post->post_name, 'coming_soon') != false) 
        {
    	?>
		<div class="oi_strange_portfolio_item oi_port_style_ii <?php echo esc_attr($col);?> <?php echo esc_attr($slugg)?>">
            <div class="oi_vc_potrfolio" style="background:url('<?php echo esc_url($large_image_url[0]); ?>')">
            	<a data-id="<?php echo $post->ID?>" href="javascript: void(0)">
