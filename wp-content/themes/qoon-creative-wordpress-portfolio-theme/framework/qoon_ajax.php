...
add_action( 'wp_ajax_qoon_ajax_request_p', 'qoon_ajax_request_p' );
add_action( 'wp_ajax_nopriv_qoon_ajax_request_p', 'qoon_ajax_request_p' );
function qoon_ajax_request_p() {
		wp_reset_postdata();
		global $post;
		$post = get_post($_GET['id']);
		$feat_image = wp_get_attachment_url( get_post_thumbnail_id($_GET['id']) );
		...
		$title  = get_the_title($_GET['id']);
		$date = get_the_date( get_option( 'date_format' ), $_GET['id']);
		$cat = get_the_terms( $_GET['id'], 'portfolio-sp-category' );
		if (isset($cat) && ($cat!='')){
			$cat_name = ''; 
			foreach($cat  as $vallue=>$key){
				$cat_name  .= ''.$key->name.', ';
