...
<div class="oi_header_side oi_full_page">
	...
	if (qoon_is_blog() || is_search()){
		...
		$p_tag = "";
		if  ($e_tag !='All'){
			$p_tag = get_term_by('name', $e_tag, 'portfolio-sp-tags');
		};

		$e_category = get_post_meta($post->ID, 'xx_sp_category', 1);
		$p_category = "";
		if  ($e_category !='All'){
			$p_category = get_term_by('name', $e_category, 'portfolio-sp-category');
		};
		...
		if($p_tag !=''){
			...
			{
				$args = array(
					'post_type' 		=> 'portfolio-sp',
					'post_status' 		=> 'publish',
					'orderby' 			=> 'date',
					'order' 			=> 'DESC',
					'tax_query' => array(
						array(
							'taxonomy' => 'portfolio-sp-tags',
							'terms'    => $p_tag->term_id,
						),
						array(
							'taxonomy' => 'portfolio-sp-category',
		...
		$thisid = current($items);

		if(sizeof($items)>0)
		{
			if(sizeof($items) == 1)
			{
				$nextid = current($items);
				$lastid = current($items);
			}
    ...
    <?php if((get_post_meta($post->ID, 'oi_ps', 1) =='creative')){?>
    	<?php if(post_password_required()){?>
			...
			<?php }else{?>
            <div class="oi_creative_p_content_partner" data-tags=<?php if  ($e_tag !='All'){ echo $e_tag; }else{echo 'all';}?> data-category=<?php if  ($e_category !='All'){ echo $e_category; }else{echo 'all';}?> data-group="<?php echo esc_attr($str);?>" data-first="<?php echo esc_attr($thisid);?>" data-last="<?php echo esc_attr($lastid);?>">
        	<a data-offset="0" data-id="<?php echo esc_attr($lastid);?>" class="oi_crea_a_partner oi_prev_c_p_partner" href="#">←</a>
        	<div class="oi_creative_p_holder_partner">
                <!-- <div class="oi_c_resize"><span>↑</span><span></span>↓</div> -->
                <?php 
                $cat = get_the_terms( $thisid, 'portfolio-sp-category' );
                if (isset($cat) && ($cat!='')){
                    $cat_name = ''; 
                ...
                <div class="oi_c_description_partner">
                	<hr>
                    <div class="oi_c_description_content_partner">
						<?php
        	...
        	<a data-offset="1" data-id="<?php echo esc_attr($nextid);?>" class="oi_crea_a_partner oi_next_c_p_partner" href="#">→</a>
        </div>
        <?php } ?>
	<?php };?>
    ...
    <?php }else{?>
    <div class="oi_image_logo"><div class="test" style="text-align: center"><a href="<?php echo esc_url(home_url('/')); ?>" class="oi_text_logo_img"><img src="<?php echo esc_url($oi_qoon_options['oi_logo_upload']['url'])?>" alt="<?php echo esc_attr(bloginfo('name'));?>" style="width: 150px;height: 204px;"/></a></div></div>
    <?php };?>
    ...
    <div class="oi_footer" style="font-size: 8px;letter-spacing: 0;">
...
<?php if(get_page_template_slug( get_the_ID() )!='portfolio-sp.php') {?>
